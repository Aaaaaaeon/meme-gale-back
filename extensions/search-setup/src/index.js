...
export default (router, { env, services, exceptions }) => {
  ...
  router.post("/meilisearch", async (req, res) => {
    try {
      ...
      await index.updateSortableAttributes(["likes", "views", "date_created"]);
      ...
      const documents = existingMemes.map((meme) => ({
        ...
        creator: `${meme.user_created?.first_name || ""} ${
          meme.user_created?.last_name || ""
        }`.trim(),
      ...
      throw new ServiceUnavailableException(
        `Erreur configuration Meilisearch: ${error.message}`
      );
    }
  });
